...
class HealthBar(pg.sprite.Sprite):
    def __init__(
        ...
        self.rect = pg.FRect(self.pos, (self.original_width, 3))
        self.update(target_obj)
        self.bg_rect = pg.FRect((self.rect.x, 0), (width, 4))
...
class PlayerHealth(pg.sprite.Sprite):
    def __init__(self, group: pg.sprite.Group, target_obj: any):
        ...
        self.heart_rect = self.heart_img.get_frect(bottomleft=(4, WIN_HEIGHT - 4))
        self.font = pg.font.Font("./res/fonts/rainy_hearts.ttf", 16)

        self.text_pos = pg.Vector2(
            self.heart_rect.topright[0] + 2, self.heart_rect.topright[1] + 1
        )
        self.offset_vec = pg.Vector2()
        self.text_surf = self.font.render(f"{self.target.health}", False, BLACK)
        self.text_shadow = self.font.render(f"{self.target.health}", False, "white")
    ...
    def draw(self, screen: pg.Surface):
        for i, j in itertools.product(range(-1, 2), range(-1, 2)):
            ...
            screen.blit(self.text_shadow, self.text_pos + self.offset_vec)
